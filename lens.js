...
$(function(){
	...
	var layer = new Kinetic.Layer();
		...
		var mouseToText = new Kinetic.Text({
			...
			fontFamily: "Calibri",
			fontSize: $("#container").height()*0.03,
			fill: "black",
			stroke: null,
			text: "",
			});
		...
		$("#lensgroup").on("mouseover", function(){
			mouseToText.setText("Húzd be a lencsét a kísérleti területre");
			layer.drawScene();
		});
		$("#objectgroup").on("mouseover", function(){
			mouseToText.setText("Kattints bármely tárgyra");
			layer.drawScene();
		});
		$("#reset").on("mouseover", function(){
			mouseToText.setText("Kattints a reset gombra a kísérleti terület visszaállításához");
			layer.drawScene();
		});
		$("#help1").on("mouseover", function(){
			mouseToText.setText("Kattints a súgó gombra ha elakadnál");
			layer.drawScene();
		});
	...
	function drawImage()
	{
		...
		if((lense_1_name!=null || lense_2_name!=null) && (object_name=="arrow" || object_name=="triangle" || object_name=="square")){
			...
			image.on("mouseover", function(){
				mouseToText.setText("Dupla kattintással a lencsére, ki tudod venni a lencsét");
				layer.drawScene();
			});
			square_object.on("mouseover", function(){
				mouseToText.setText("Mozgasd a síkban a testet hogy megkapd a képet");
				layer.drawScene();
			});
